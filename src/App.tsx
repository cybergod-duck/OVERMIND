...
function App() {
  ...
  // Auto-import from overmind-secrets.json (generated by seed-vault.cjs) on first launch
  useEffect(() => {
    const alreadyImported = localStorage.getItem('overmind_v4_seeded')
    if (alreadyImported) return
    fetch('/overmind-secrets.json')
      .then(res => {
        if (!res.ok) throw new Error('No seed file')
        return res.json()
      })
      .then((seedSecrets: any[]) => {
        if (!Array.isArray(seedSecrets) || seedSecrets.length === 0) return
        setSecrets(prev => {
          // Only add keys that aren't already in the vault (by value)
          const existingValues = new Set(prev.map(s => s.value))
          const newOnes = seedSecrets.filter(s => !existingValues.has(s.value))
          if (newOnes.length === 0) return prev
          localStorage.setItem('overmind_v4_seeded', '1')
          return [...prev, ...newOnes]
        })
      })
      .catch(() => {
        // No seed file — that's fine, user can import manually
      })
  }, [])
